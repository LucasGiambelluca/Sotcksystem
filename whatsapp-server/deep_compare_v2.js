...
async function run() {
    ...
    console.log('VPS Columns:', vpsCols.join(', '));
    
    const missingInVps = localCols.filter(c => !vpsCols.includes(c));
    const missingInLocal = vpsCols.filter(c => !localCols.includes(c));
    
    console.log('\nMissing in VPS (should add these to production):', missingInVps.join(', '));
    console.log('Missing in Local (extra in production):', missingInLocal.join(', '));
}
run();
