...
import { Users, Plus, Link, RefreshCw, Send, Shield, UserPlus, X, Check, Radio } from 'lucide-react';
import { toast } from 'sonner';
import { supabase } from '../supabaseClient';
import { broadcastMessage } from '../services/whatsappService';
...
export default function WhatsAppGroups() {
  ...
  const [broadcastClients, setBroadcastClients] = useState<string[]>([]);
  ...
  function toggleBroadcastSelectAll() {
    if (broadcastSelectAll) {
      ...
      setBroadcastClients(clients.filter(c => c.phone).map(c => c.phone!));
    }
    setBroadcastSelectAll(!broadcastSelectAll);
  }

  async function handleBroadcast() {
    if (!broadcastText.trim() || broadcastClients.length === 0) return;
    setBroadcastSending(true);
    try {
      await broadcastMessage(broadcastClients, broadcastText);
      toast.success(`📢 Enviando a ${broadcastClients.length} contactos...`);
      setBroadcastText('');
      setBroadcastClients([]);
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="mb-8 bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-lg shadow-sm border border-blue-200">
          <h3 className="text-lg font-semibold mb-2 flex items-center gap-2">
            ...
            Difusión Individual (Broadcast)
          </h3>
          <p className="text-sm text-gray-500 mb-4">Envía un mismo mensaje a múltiples contactos de forma individual (1:1).</p>
          <button
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-lg flex flex-col max-h-[85vh]">
                ...
                <div className="p-4 border-b">
                    ...
                    <textarea 
                        ...
                        placeholder="Escribí el mensaje que se enviará a cada contacto..."
                        className="w-full px-3 py-2 border rounded-lg resize-none h-24 focus:ring-2 focus:ring-blue-500"
                    />
                </div>
                ...
                <div className="px-4 py-2 border-b bg-gray-50 flex items-center justify-between">
                    <span className="text-sm text-gray-600">
                        {broadcastClients.length} de {clients.filter(c => c.phone).length} seleccionados
                    </span>
                    <button
                        onClick={toggleBroadcastSelectAll}
                        className="text-sm text-blue-600 hover:text-blue-800 font-medium"
                    >
                        {broadcastSelectAll ? 'Deseleccionar todos' : 'Seleccionar todos'}
                    </button>
                </div>
                        ...
                        {clients.filter(c => c.phone).map(client => (
                            <div key={client.id}
                                className={`flex items-center justify-between p-3 rounded-lg cursor-pointer border ${
                                    broadcastClients.includes(client.phone!) 
                                        ? 'bg-blue-50 border-blue-500' 
                                        : 'hover:bg-gray-50 border-gray-200'
                                }`}
                                onClick={() => {
                                    const phone = client.phone!;
                                    if (broadcastClients.includes(phone)) {
                                        setBroadcastClients(broadcastClients.filter(p => p !== phone));
                                    } else {
                                        setBroadcastClients([...broadcastClients, phone]);
                                    }
                                }}
                            >
                                <div>
                ...
                <div className="p-4 border-t flex justify-end gap-3 bg-gray-50">
                    <span className="text-sm text-gray-500 self-center mr-auto">
                        {broadcastClients.length} destinatarios
                    </span>
                    <button
                        onClick={handleBroadcast}
                        disabled={broadcastSending || !broadcastText.trim() || broadcastClients.length === 0}
                    ...
                    >
                        ...
                        {broadcastSending ? 'Enviando...' : `Enviar a ${broadcastClients.length}`}
                    </button>
                </div>
            </div>
        </div>
