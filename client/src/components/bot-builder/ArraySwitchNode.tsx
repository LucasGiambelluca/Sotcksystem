...
import { Spline, Plus, Trash2, Key } from 'lucide-react';
    ...
    <div className="bg-white rounded-lg shadow-lg border-2 border-indigo-200 w-72">
      <div className="bg-indigo-500 text-white p-2 rounded-t-lg flex items-center justify-between">
        <div className="flex items-center gap-2">
            <Spline size={16} />
            <span className="font-medium text-sm">Switch Array / Palabras</span>
        </div>
      ...
      <div className="p-3 bg-indigo-50">
        <div className="mb-3">
          <label className="block text-[10px] font-bold text-indigo-700 mb-1 uppercase tracking-tighter">Variable Array a evaluar:</label>
          <div className="relative">
            <Key size={10} className="absolute left-2 top-2 text-indigo-400" />
            <input
              type="text"
              className="w-full text-xs pl-6 p-1.5 border rounded-md font-mono bg-white focus:ring-1 focus:ring-indigo-500 outline-none"
              value={data.variable}
        ...
        <div className="space-y-2 mb-3 max-h-48 overflow-y-auto pr-1">
          {cases.map((c: any, index: number) => (
            <div key={index} className="flex items-center gap-2 group">
              <input
                type="text"
                className="flex-1 text-xs p-1.5 border rounded-md bg-white focus:ring-1 focus:ring-indigo-500 outline-none"
                value={c.value}
              ...
              <Handle 
                ...
                id={c.handle}
                style={{ top: 145 + (index * 36), background: '#6366f1' }}
        ...
        <div className="mt-4 pt-2 border-t border-indigo-100 flex justify-between items-center pr-2">
            <span className="text-[10px] text-gray-400 font-bold">NINGUNO CONTENIDO</span>
            <Handle 
              type="source" 
              position={Position.Right} 
              id="default"
